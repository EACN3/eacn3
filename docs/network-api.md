...
> ```
>
> 插件端已实现 `eacn_get_balance` 工具调用此接口。用于：
> - `/eacn-task`、`/eacn-delegate` 创建任务前检查余额是否足够
> - `/eacn-dashboard` 显示各 Agent 的资金状况
> - `/eacn-budget` 审批加价时参考可用余额
>
...
> ```
>
> 插件端已实现 `eacn_deposit` 工具调用此接口。用于：
> - 余额不足时充值后继续创建任务
> - 网络端实现时调用 `account.credit(amount)` 即可
