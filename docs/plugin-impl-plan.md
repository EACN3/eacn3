...
| `registry.ts` | 做三件事：校验（几个 if）、持久化（state.addAgent）、调网络（networkClient.register）。三行代码不值得单独一个模块，内联到 eacn_register_agent handler。 |
| `matcher.ts` | 文档里 matcher 做"本地匹配"。plugin 里就是 eacn_create_task 时先查 state 里有没有匹配的本地 Agent，几行代码内联。 |
| `logger.ts` | 文档里 logger 做"状态变更记录"。plugin 里就是在 submit_result/reject_task 等 handler 里顺便调 eacn_report_event，一行代码。 |
...
- `drainEvents()` — 返回并清空 buffer（供 eacn_get_events 调用）
- 自动 ping 保活
- 连接断开自动重连
...
特殊逻辑（内联，不单独模块）：
- `eacn_register_agent`：组装 AgentCard（原 adapter）+ 校验（原 registry）+ 调网络注册 + 存 state + 建 WS 连接
- `eacn_create_task`：先查 state 里本地 Agent 有没有匹配的（原 matcher），没有则走网络
- `eacn_submit_result` / `eacn_reject_task`：执行后自动调 reportEvent（原 logger）
- `eacn_get_events`：drain ws-manager 的 buffer
